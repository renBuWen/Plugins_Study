...
Resize a column by hovering over the separator and dragging.  You can also auto resize it to the largest entry by double clicking the bar instead of dragging.
...
In Console Pro, press the button labeled "Remote" in the top right.
...
# Jump To Line Callback
If you'd like to override the jump to line functionality in Console Pro so you can jump to lines to special editors, simply do this in your editor script:

ConsoleProWindow.jumpToFileCallback += TestCallback;

static void TestCallback(string inPath, int inLine)
{
	Debug.LogError("TestCallback: " + inPath + ":" + inLine);
}
